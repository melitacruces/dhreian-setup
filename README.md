...
[Abrir MiSetup](https://misetup.melitacruces.com)
...
2. Copia `.env.example` como `.env.local` y configura la instalación:
   ...
   GOOGLE_SITE_VERIFICATION=token-opcional-de-search-console
...
Para acelerar la indexación del dominio público, verifica la propiedad en Google
Search Console mediante `GOOGLE_SITE_VERIFICATION` y envía
`https://tu-dominio.com/sitemap.xml`. La aplicación incluye metadatos canónicos,
datos estructurados, rutas indexables por sección y el archivo `robots.txt`.

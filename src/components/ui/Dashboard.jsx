...
export default function Dashboard({
  ...
  const [activeTab, setActiveTab] = useState(
    initialSection || initialSections[0]?.slug || 'core'
  );
  const [routeSection, setRouteSection] = useState(
    initialRouteSection || null
  );
  ...
  const activeSectionTitle = sections.find(
    section => section.slug === activeTab
  )?.title;

  const updateDashboardLocation = (
    view,
    section,
    { replace = false } = {}
  ) => {
    const path = getDashboardPath(view, section);
  ...
  const navigateToView = view => {
    setActiveView(view);
    setRouteSection(null);
    updateDashboardLocation(view);
  };
  ...
  const navigateToSection = section => {
    ...
    updateDashboardLocation('inventory', section);
  };
  ...
  useEffect(() => {
    ...
    setActiveTab(initialSection || initialPayload.sections[0]?.slug || 'core');
    setRouteSection(initialRouteSection || null);
  }, [
    initialPayload,
    initialEditorMode,
    initialView,
  ...
  useEffect(() => {
    const handlePopState = () => {
      ...
      if (
        route.section &&
        sections.some(section => section.slug === route.section)
      ) {
  ...
  useEffect(() => {
    const pageMetadata = getDashboardDocumentMetadata(
      activeView,
      activeView === 'inventory' && routeSection ? activeSectionTitle : null
    );
    const path =
      activeView === 'inventory' && routeSection
        ? getDashboardPath('inventory', activeTab)
        : pageMetadata.path;
  ...
  }, [activeView, activeTab, activeSectionTitle, routeSection]);
  ...
  useEffect(() => {
    if (sections.length > 0 && !sections.some(section => section.slug === activeTab)) {
      const fallbackSection = sections[0].slug;
      setActiveTab(fallbackSection);
      if (activeView === 'inventory') {
        setRouteSection(fallbackSection);
        updateDashboardLocation('inventory', fallbackSection, { replace: true });
      }
    }
  }, [sections, activeTab, activeView]);
  ...
  return (
    ...
    >
      ...
      <Sidebar
        ...
        navigateSection={navigateToSection}
